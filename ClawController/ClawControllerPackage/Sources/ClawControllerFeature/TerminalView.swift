                ...
                if let result = entry.result {
                    ...
                    if !result.data.isEmpty {
                        VStack(alignment: .leading, spacing: 2) {
                            ForEach(Array(result.data.keys), id: \.self) { key in
                                HStack {
                                    Text(key)
