...
func resolvePid(_ pid: Int?, x: CGFloat? = nil, y: CGFloat? = nil) -> Int {
    ...
    if let cx = x, let cy = y {
        ...
        let error = AXUIElementCopyElementAtPosition(
            systemWideElement, Float(cx), Float(cy), &element)
        if error == .success, let element = element {
            ...
            if pidError == .success && inferredPid > 0 {
                debugLog(
                    "log: resolvePid: inferred PID \(inferredPid) from coordinates (\(cx), \(cy))\n",
                    stderr)
...
func adjustDragStartCoordinateIfNeeded(x: Double, y: Double) -> (Double, Double) {
    ...
    if error == .success, let element = element {
        ...
        while true {
            var role: CFTypeRef?
            if AXUIElementCopyAttributeValue(currentElement, kAXRoleAttribute as CFString, &role)
                == .success,
                let roleStr = role as? String, roleStr == "AXWindow"
            {
                ...
                if AXUIElementCopyAttributeValue(
                    currentElement, kAXPositionAttribute as CFString, &positionRef) == .success,
                    AXUIElementCopyAttributeValue(
                        currentElement, kAXSizeAttribute as CFString, &sizeRef) == .success
                {
                    ...
                    if relativeY >= 0 && relativeY < 40 {
                        ...
                        {
                            debugLog(
                                "log: adjustDragStartCoordinate: keeping original coordinate for tab role \(roleStr)\n",
                                stderr)
                            return (x, y)
                        }
                        ...
                        debugLog(
                            "log: adjustDragStartCoordinate: shifting drag start from (\(x), \(y)) to safe title bar area (\(newX), \(newY))\n",
                            stderr)
                        return (newX, newY)
                    }
                }
            ...
            if AXUIElementCopyAttributeValue(
                currentElement, kAXParentAttribute as CFString, &parentRef) == .success,
