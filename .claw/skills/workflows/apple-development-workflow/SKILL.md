...
11. **Continuous Code Verification (CRITICAL):** After creating or modifying any file, you MUST immediately verify it using all possible checks (e.g., `xcode-bridge` tools, `xcodebuild`, IDE feedback). Furthermore, at the end of every major stage in your `task.md` plan, you MUST perform a comprehensive compilation and verification of the entire generated codebase. Do NOT mark a task as completed or report that the system is ready if the code has not been strictly verified and compiled without errors.
12. **Automatic Code Signing (CRITICAL):** When generating or modifying `project.yml`, you MUST always configure the `DEVELOPMENT_TEAM`. Check if the `XCODE_TEAM_ID` environment variable is available (or ask the user if it's missing). In the `project.yml`, inject it under `settings.base.DEVELOPMENT_TEAM: "${XCODE_TEAM_ID}"` (or the literal value if known). Never leave `DEVELOPMENT_TEAM: ""` empty.
...
## 4. Premium SwiftUI Design Guidelines (Apple HIG for iOS 26)
When writing UI code in SwiftUI, you MUST specifically target the **iOS 26+ SDK** and completely abandon basic generic designs, striving instead for "Apple Design Awards" quality. Actively leverage the most modern iOS 26 exclusive APIs and modifiers.
1. **Glassmorphism & Depth:** Do NOT use flat, boring colors for backgrounds, navigation bars, or overlays. Actively use `.background(.ultraThinMaterial)` or `.regularMaterial` for floating elements, status cards, and bottom sheets to blend beautifully with the content behind them.
2. **Dynamic Backgrounds:** Avoid solid white/black backgrounds. Use `MeshGradient` (iOS 18-26) or subtle, multi-stop `.linearGradient` for primary view backgrounds to give the app a modern, fluid, and expensive feel.
3. **Advanced Iconography:** ALWAYS use SF Symbols (version 5+). Never use static, dead icons. Apply symbol modifiers like `.symbolEffect(.bounce, value: state)`, `.symbolEffect(.pulse)`, or `.symbolVariant(.fill)` to make icons feel alive and react to user interaction.
4. **Sensory & Haptic Feedback:** EVERY interactive element (buttons, toggles, map pins, success/error states) MUST have a tactile physical response. Use `.sensoryFeedback(.impact, trigger: ...)` or `UIImpactFeedbackGenerator` for all interactive components.
5. **Modern Layout & Fluid Animations:** Use `scrollTransition` for list items entering/leaving the screen so they scale or fade naturally. Use smooth physics-based animations like `withAnimation(.spring(response: 0.4, dampingFraction: 0.7))` instead of basic linear or ease-in-out animations.
6. **Typography & Shadows:** Use Apple's native dynamic typography with modern designs (e.g., `.font(.system(.title2, design: .rounded, weight: .bold))`). Apply soft, highly diffuse shadows (`.shadow(color: .black.opacity(0.1), radius: 15, x: 0, y: 8)`) to lift interactive cards above materials.
