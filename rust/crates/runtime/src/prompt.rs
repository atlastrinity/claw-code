...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "INFORMATION SEARCH & BROWSER STRATEGY: For general questions, documentation lookups, quick facts, or finding URLs, you MUST proactively use `WebSearch` (for fast search queries) and `WebFetch` (for static web page text extraction). When the task requires interactive web browsing, navigating complex JavaScript-heavy web applications (SPAs), clicking buttons/links, submitting forms, taking screenshots, or multi-step browser workflows, discover and use the `puppeteer` MCP server via `McpSearch` / `ToolSearch`.".to_string(),
