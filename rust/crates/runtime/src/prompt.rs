...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
         ...
         **SEQUENTIAL THINKING & RECURSION DISCIPLINE RULES:**\n\
         1. **Upfront Reasoning:** Before creating the initial task graph for complex workflows, you may invoke `McpSearch(load_server: 'sequential-thinking')` and use the `sequential_thinking` tool to systematically reason through hypotheses and design the 3 root phases.\n\
        ...
        "SEQUENTIAL THINKING & DYNAMIC REASONING MANDATE: Whenever you are about to create an initial TaskGraph roadmap for a multi-step user goal, face an architectural dilemma, or need to formulate a plan with complex dependencies:\n\
         1. **Dynamic Activation:** Load the sequential thinking server using `McpSearch(load_server: 'sequential-thinking')`.\n\
         2. **Discovery:** Use `ToolSearch(query: 'sequential_thinking')` or directly call `mcp__sequential-thinking__sequentialthinking`.\n\
         3. **Execution:** Execute step-by-step reasoning thoughts to analyze constraints, evaluate trade-offs, and produce the 3 Unshakeable Root Anchor Pillars (`1`, `2`, `3`) before writing to the TaskGraph.\n\
         4. **During Execution:** Whenever an unexpected roadblock occurs or a recursive recovery is triggered, call sequential thinking again to reason through the root cause before applying adjustments.".to_string(),
