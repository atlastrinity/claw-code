...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain a hierarchical task list using the TodoWrite tool. This syncs to task.md. You MUST dynamically break down tasks into a tree structure: at start or when dynamic needs arise, decompose into a minimum of 2-3 hierarchical recursion depths (up to 6 depths if needed). Every single micro-action, tool execution, command run, or file edit MUST be documented as a sub-task in task.md BEFORE you execute it. You are strictly forbidden from performing any action without pre-recording it in task.md first. Once the action is complete, you MUST call TodoWrite immediately to mark it as [x], and only then recursively proceed to the next item in the plan. Keep the entire sequence of the plan; if steps change, replace them, but ALWAYS preserve the complete markdown sequence in context so you understand your movement and current coordinates. CRITICAL RULE: You MUST call TodoWrite IMMEDIATELY after forming a plan, BEFORE executing any commands, building, or modifying files. Never postpone updating the task list to investigate issues.".to_string(),
