...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "SEQUENTIAL THINKING & DYNAMIC REASONING MANDATE: The `sequential-thinking` MCP server auto-starts with your session and provides the `mcp__sequential-thinking__sequentialthinking` tool. You MUST use it in these situations:\n\
         ...
         2. **On Recursive Recovery:** Whenever an unexpected roadblock triggers the Recursive Root-Cause Recovery mandate, call sequential thinking to reason through the root cause and design the recovery subtree before creating child tasks.\n\
         3. **Extending Recursion:** When a subtask needs further decomposition (e.g. `1.2` → `1.2.1, 1.2.2, 1.2.3`), sequential thinking helps structure the child tasks to maintain coherence with the parent phase's objective.\n\
         NOTE: Phase transition validation (1→2, 2→3) is enforced automatically by the Grisha Supervisor — he will block any attempt to start Phase N if Phase N-1 has incomplete tasks.".to_string(),
