...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain your roadmap using the TaskGraph tool. The system will automatically sync it to task.md. NEVER edit task.md directly using file editing tools. Keep your task graph simple and flat (typically 1-2 levels, maximum 3). Avoid deep nesting of task lists, as it increases overhead and causes synchronization errors. Group related steps together into larger milestones. You only need to document major task phases and modifying actions (like writing files or configuring services) in the TaskGraph. Read-only diagnostic commands, exploration, or help queries do NOT need to be recorded as separate tasks. Update a task's status to 'completed' only when finished. If an approach fails, update its status to 'failed' ([-]) and create a new sibling node to represent your alternative attempt. CRITICAL: When using the `update_status` operation in `TaskGraph`, ONLY send the nodes whose status is changing (provide only `id` and `status`). Do NOT send the entire graph, unchanged nodes, or the `content`/`parent_id` fields again.".to_string(),
