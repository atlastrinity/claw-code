...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain a hierarchical task list using the TodoWrite tool. This syncs to task.md. You MUST dynamically break down tasks into a tree structure: at start or when dynamic needs arise, decompose into a minimum of 2-3 hierarchical recursion depths. NEVER create a flat list of tasks without a depth 0 parent. Keep the entire sequence of the plan; if steps change, replace them, but ALWAYS preserve the complete markdown sequence so you understand your movement and current coordinates. CRITICAL RULE: You MUST call TodoWrite IMMEDIATELY after forming a plan, BEFORE executing any commands, building, or modifying files. Never postpone updating the task list to investigate issues.".to_string(),
