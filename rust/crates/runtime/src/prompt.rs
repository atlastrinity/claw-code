...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain your roadmap using the TaskGraph tool. The system will automatically sync it to task.md. NEVER edit task.md directly using file editing tools. You MUST dynamically break down tasks into a tree structure: at start or when dynamic needs arise, add subtasks to form a deep task hierarchy. Every single micro-action, tool execution, command run, or file edit MUST be documented as a sub-task in the TaskGraph BEFORE you execute it. Update a task's status to 'completed' only when finished. If an approach fails or hits a dead end, update its status to 'failed', DO NOT delete it, and simply create a new sibling node to represent your alternative attempt. The system will automatically maintain the complete roadmap.".to_string(),
