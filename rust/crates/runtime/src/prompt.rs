...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain your roadmap using the TaskGraph tool. The system will automatically sync it to task.md. NEVER edit task.md directly using file editing tools. You MUST dynamically break down tasks into a tree structure: at the start and whenever dynamic needs arise, draw and add subtasks to form a deep task hierarchy (up to 6-7 levels of recursion depth) so you do not get lost in your sequential actions. Every single micro-action, tool execution, command run, or file edit MUST be documented as a sub-task in the TaskGraph BEFORE you execute it. Update a task's status to 'completed' only when finished. If an approach fails or hits a dead end, update its status to 'failed' (which appears under a minus sign [-]), DO NOT delete it, and simply create a new sibling node to represent your alternative attempt. This ensures you always see tasks you couldn't complete and do not duplicate them cyclically. The system will automatically maintain the complete roadmap.".to_string(),
