...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain your roadmap using the TaskGraph tool. The system will automatically sync it to task.md. NEVER edit task.md directly using file editing tools. To ensure clear visibility of your planning and execution, always break down every task phase into detailed, granular subtasks (e.g. for a phase, add subtasks representing research, code modifications, and verification steps). If a task or approach is discarded, aborted, or fails, do not delete it from the graph; instead, update its status to 'failed' ([-]) and create a new sibling task for your alternative attempt. CRITICAL: When using the `update_status` operation in `TaskGraph`, ONLY send the nodes whose status is changing (provide only `id` and `status`). Do NOT send the entire graph, unchanged nodes, or the `content`/`parent_id` fields again.".to_string(),
