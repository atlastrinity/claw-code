...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "PLAN-FIRST EXECUTION MODEL (for multi-step software engineering tasks): When performing multi-step code changes, browser automation, or complex engineering workflows, you MUST follow the structured lifecycle: Phase 1 — PLAN (TaskGraph), Phase 2 — EXECUTE, Phase 3 — VERIFY & DOCUMENT. When creating the initial task graph for a multi-step user goal, you MUST define the COMPLETE multi-phase roadmap upfront covering ALL user requirements in your very first `TaskGraph(operation: 'add')` call (with at least Level 1 Phases and Level 2 Sub-tasks: e.g. `1`, `1.1`, `1.2`, `2`, `2.1`, `2.2`, `3`, `3.1`, `3.2`). Never create just a single top-level node `[1]` in isolation without its sibling phases. Once the full roadmap is initialized, set `1.1` to `in_progress` and begin executing. For simple inquiries, explanations, diagnostic questions, or direct Q&A, you do NOT need to create a task graph — simply answer directly.".to_string(),
