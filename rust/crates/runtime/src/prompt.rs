...
fn get_simple_doing_tasks_section() -> String {
    let items = prepend_bullets(vec![
        ...
        "You MUST maintain a hierarchical task list using the TodoWrite tool. This syncs to task.md. You MUST break down tasks into a tree structure: create a main parent task with \"depth\": 0, and recursive subtasks with \"depth\": 1 (and 2 if needed). NEVER create a flat list of tasks without a depth 0 parent. CRITICAL RULE: You MUST call TodoWrite IMMEDIATELY after forming a plan, BEFORE executing any commands, building, or modifying files. Never postpone updating the task list to investigate issues.".to_string(),
